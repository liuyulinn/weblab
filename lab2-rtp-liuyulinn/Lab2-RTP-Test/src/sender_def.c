        ...
        if(count == PAYLOAD_SIZE -1 || (feof(f) && count != 0)) //将这一段打包发送出去
        {
            ...
            while(1)
            {
                ...
                else if(sender_seq_num < (unacked_seq_num + sender_window_size)) //可以发送
                {
                    ...
                    count = 0;
                    if((result = rtp_sendto(sender_sock, (void*)sender_buffer, sender_rtp->length + sizeof(rtp_header_t), 0, (struct sockaddr*)&receiver_addr, sizeof(struct sockaddr))) < 0)
                    {
                        continue; //传输出现问题
                    }

                   //printf("Sender: send %d, unacked %d\n", sender_seq_num, unacked_seq_num);
                    ++ sender_seq_num;
                    break;
                }
            }
